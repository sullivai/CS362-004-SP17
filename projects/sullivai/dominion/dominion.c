...
int scoreFor (int player, struct gameState *state) {
  ...
  int score = 0;
    ...
    for (i = curse; i <= treasure_map; i++){
        deckTotal += fullDeckCount(player,i,state);
    }
    deckTotal /= 10;
  ...
  for (i = 0; i < state->handCount[player]; i++)
    {
      ...
      //if (state->hand[player][i] == gardens) { score = score + ( deckTotal / 10 ); };
      if (state->hand[player][i] == gardens) { score = score + deckTotal; };
    }
  ...
  for (i = 0; i < state->discardCount[player]; i++)
    {
      ...
      //if (state->discard[player][i] == gardens) { score = score + ( deckTotal / 10 ); };
      if (state->discard[player][i] == gardens) { score = score + deckTotal; };
    }
  ...
  for (i = 0; i < state->deckCount[player]; i++)
    {
      ...
      //if (state->deck[player][i] == gardens) { score = score + ( deckTotal / 10 ); };
      if (state->deck[player][i] == gardens) { score = score + deckTotal; };
    }
...
// Play council room
void play_Council_room(struct gameState *state, int currentPlayer, int handPos){
  int i;
  //+4 Cards
  for (i = 0; i < 4; i++)
  {
...
// Play mine
int play_Mine(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2){
  int i,j;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case feast:
      play_Feast(state, currentPlayer, choice1);
      return 0;
    ...
    case mine:
      play_Mine(state, currentPlayer, handPos, choice1, choice2);
      return 0;
